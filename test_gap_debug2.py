...
class MyVP(VideoProcessor):
    def process_frame(self, frame):
        ...
        if self.frame_count == 1162:
            print('--- Frame 1162 ---')
            # Let's run gap_analyzer manually to debug
            ...
            for i, dist_bev in enumerate(distances_bev):
                ...
                print(f'Gap {i}: dist_m={dist_meters:.2f} dist_bev={dist_bev:.2f}')
                if dist_meters < self.gap_analyzer.gap_threshold_meters:
                    ...
                    if self.gap_analyzer._midpoint_in_exclusion_zone(tuple(mid_orig.tolist())):
                        ...
                        print('  -> PASSED!')
        self._last_detections = self._cached_detections if hasattr(self, '_cached_detections') else getattr(self, 'last_detections', [])
        return out
        
    def _detect_and_track(self, frame):
        dets = super()._detect_and_track(frame)
