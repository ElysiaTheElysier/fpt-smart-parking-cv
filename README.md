...
# \* `/scripts/` - Các script lẻ.

# \* `/backend/` - Code FastAPI, WebSockets.

# \* `/frontend/` - Code ReactJS, UI Dashboard.

# \* `/notebooks/` - test nháp thuật toán bằng Jupyter Notebook.

# 

# \## Quy tắc rẽ nhánh 

# Mỗi task mới, ae tự tạo một nhánh từ nhánh `dev` với cú pháp: `<loai>/<ten-task>`
...
# 3\. Nhắn lên group Zalo/Discord để Dương (PM) hoặc người liên quan vào review code.

# 4\. Có ít nhất 1 người Approve mới được bấm Merge vào nhánh `dev`.

# 

# \* File `.gitignore` đã chặn thư mục `venv/`, `\_\_pycache\_\_/` và `node\_modules/`. Tuyệt đối không cố tình force push các thư mục này lên git.
